...
def main():
    ...
    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        ...
        for input_file in input_files:
            p=multiprocessing.Process(target=read_file, args=(input_file, shared_list))
            jobs.append(p)
            if(len(jobs) % 10 ==0):
                time.sleep(20)
            p.start()
        ...
        # time.sleep(20)
        # num=0
        for i,doc in enumerate(shared_list):
            docs.add_document(doc)
            if(i % 10000 == 0):
                print('have processed {} docs'.format(i))
        # while(shared_list): 
        #     docs.add_document(shared_list.pop(0))
        #     num +=1
        #     if(num % 10000 == 0):
        #         print('have processed {} docs'.format(num))
